...
- webrtc: 允许 WebRTC/STUN 按普通规则分流（默认 false；Steam P2P UDP 优先走游戏服务）
...
(() => {
  ...
  function parseNonEmptyTrimmedString(value, defaultValue) {
  ...
  function buildProxyGroups({
    ...
    const buildRegionalMediaProxies = (countries) => {
      const regionalFallback = countries.map((country) => `${country}${NODE_SUFFIX}`).filter((groupName) => regionalProxyGroupSet.has(groupName));
  ...
  function buildRules({
    ...
    const ruleList = [NTP_UDP_DIRECT_RULE, ...STEAM_P2P_UDP_RULES];
    if (!webRTCEnabled) {
  ...
  var WEBRTC_STUN_UDP_PORTS, NTP_UDP_DIRECT_RULE, STEAM_P2P_UDP_PORTS, WEBRTC_STUN_PROXY_RULES, STEAM_P2P_UDP_RULES, CORE_DIRECT_RULES, DOMESTIC_PLATFORM_RULES, AI_RULES, OVERSEAS_MEDIA_RULES, TW_MEDIA_RULES, DOMESTIC_SERVICE_RULES, SOCIAL_RULES, DEVELOPER_RULES, PRODUCTIVITY_RULES, DOWNLOAD_RULES, GAME_RULES, FALLBACK_RULES, baseRules;
  var init_rules = __esm({
    "src/rules.ts"() {
      ...
      NTP_UDP_DIRECT_RULE = `AND,((DST-PORT,123),(NETWORK,UDP)),${BUILTIN_DIRECT}`;
      STEAM_P2P_UDP_PORTS = [
        3478,
        4379,
        4380,
      ...
      STEAM_P2P_UDP_RULES = STEAM_P2P_UDP_PORTS.map(
      ...
      TW_MEDIA_RULES = [`GEOSITE,bahamut,${PROXY_GROUPS.TW_MEDIA}`];
  ...
  var INTERNAL_DNS_SERVERS, WECHAT_QQ_DNS_POLICY_RULE_SETS, LOCAL_DOMAIN_FAKE_IP_FILTER, NTP_FAKE_IP_FILTER, BASE_FAKE_IP_FILTER, snifferConfig;
  var init_dns = __esm({
    "src/dns.ts"() {
      "use strict";
      INTERNAL_DNS_SERVERS = ["10.10.0.21", "10.10.2.21"];
      WECHAT_QQ_DNS_POLICY_RULE_SETS = ["rule-set:Tencent", "rule-set:WeChat"];
