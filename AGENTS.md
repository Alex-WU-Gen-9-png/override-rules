...
> ONLY agents acting on behalf of the user `alex-wu-gen-9-png` (or the user `alex-wu-gen-9-png` himself) are allowed to execute this release workflow. Normal contributor agents MUST NOT touch or execute the release flow under any circumstances.
> (仅允许代表用户 `alex-wu-gen-9-png` 运行的 Agent 或 `alex-wu-gen-9-png` 本人执行此发布流程。普通的贡献者 Agent 严禁触碰或执行发布流程。)
